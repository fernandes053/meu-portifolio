...
- `GET /api/commit-count`: usa cache quando estiver fresco (<24h). Se estiver vazio/expirado, sincroniza no GitHub automaticamente. A sincronização tenta primeiro a busca global de commits por autor (`author:<username>`), depois fallback para contribuição de perfil e por fim fallback por repositórios próprios.
